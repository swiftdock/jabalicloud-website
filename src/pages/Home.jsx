...
function Hero() {
  ...
  return (
    <section style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', background:'#f8fafc', overflow:'hidden', paddingTop:80 }}>
      {/* Subtle light mesh — blue tint top-right */}
      <div style={{ position:'absolute', top:0, right:0, width:700, height:700, pointerEvents:'none',
        background:'radial-gradient(ellipse 70% 60% at 80% 20%, rgba(37,99,235,.07) 0%, transparent 65%)'}} />
      <div style={{ position:'absolute', bottom:0, left:0, width:500, height:500, pointerEvents:'none',
        background:'radial-gradient(ellipse 60% 50% at 20% 80%, rgba(0,184,148,.05) 0%, transparent 65%)'}} />
      {/* Fine dot grid */}
        ...
        <div style={{ maxWidth:720 }}>
          <Reveal>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, marginBottom:28,
              background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:999, padding:'8px 18px' }}>
              <span style={{ width:7, height:7, borderRadius:'50%', background:'#10b981', flexShrink:0, animation:'breathe 3s ease-in-out infinite' }} />
              <span style={{ fontSize:13, fontWeight:500, color:'#1e40af' }}>Trusted by 10,000+ websites across Africa</span>
            </div>
          </Reveal>

          <Reveal delay={80}>
            <h1 style={{ fontWeight:900, color:'#0f172a', marginBottom:20, lineHeight:.95, letterSpacing:'-0.04em',
          ...
          <Reveal delay={160}>
            <p style={{ fontSize:'clamp(16px,2vw,20px)', color:'#475569', maxWidth:540, marginBottom:36, lineHeight:1.65 }}>
              Blazing-fast LiteSpeed servers, 99.9% uptime guarantee, and real people supporting you around the clock.
            </p>
          </Reveal>

          {/* Domain search */}
          <Reveal delay={240}>
            <div style={{ display:'flex', flexDirection:'column', gap:12, maxWidth:580, marginBottom:28 }}>
              <div style={{ display:'flex', gap:10, padding:6, background:'#fff', border:'1.5px solid #e2e8f0',
                borderRadius:16, boxShadow:'0 2px 8px rgba(15,23,42,.06)', flexWrap:'wrap' }}>
                <input value={domain} onChange={e=>setDomain(e.target.value)}
                  onKeyDown={e=>{ if(e.key==='Enter') window.location.href=`/domains?q=${domain}`; }}
                  placeholder="yourbusiness.com"
                  style={{ flex:1, minWidth:160, padding:'12px 16px', fontSize:15, color:'#0f172a',
          ...
          <Reveal delay={320}>
            <div style={{ display:'flex', flexWrap:'wrap', gap:12, marginBottom:56 }}>
              {[['🔒','Free SSL'],['⚡','99.9% Uptime'],['🔄','Free Migration'],['💬','24/7 Support'],['↩️','30-Day Guarantee']].map(([icon,text]) => (
                <div key={text} style={{ display:'flex', alignItems:'center', gap:6, fontSize:13, fontWeight:500,
                  color:'#64748b', background:'#fff', border:'1px solid #e2e8f0', borderRadius:999, padding:'6px 14px' }}>
          ...
          <Reveal delay={400}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
              {[['10K+','Websites Hosted'],['99.9%','Uptime SLA'],['< 2min','Avg Support Reply'],['4.9★','Customer Rating']].map(([val,label]) => (
                <div key={label} className="glass-card" style={{ padding:'20px 16px', textAlign:'center' }}>
                  <div className="grad" style={{ fontSize:22, fontWeight:900, letterSpacing:'-0.03em', marginBottom:4 }}>{val}</div>
...
function LogoStrip() {
  return (
    <section style={{ background:'#fff', borderTop:'1px solid #f1f5f9', borderBottom:'1px solid #f1f5f9', padding:'32px 24px' }}>
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        <p style={{ textAlign:'center', fontSize:11, fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', color:'#cbd5e1', marginBottom:24 }}>Powered by industry leaders</p>
...
function HostingPlans() {
  const plans = [
    { icon:'🌐', label:'Shared Hosting', href:'/shared-hosting', price:'From $1.99/mo', accent:'#2563eb', bg:'#eff6ff', border:'#bfdbfe',
    ...
    { icon:'⚡', label:'VPS Hosting', href:'/vps', price:'From $9.99/mo', accent:'#7c3aed', bg:'#f5f3ff', border:'#ddd6fe',
    ...
    { icon:'🖥️', label:'Dedicated Servers', href:'/dedicated', price:'From $89/mo', accent:'#0891b2', bg:'#ecfeff', border:'#a5f3fc',
      ...
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <Reveal>
          <div style={{ marginBottom:56 }}>
            <div className="eyebrow" style={{ marginBottom:12 }}>Hosting Solutions</div>
            <h2 style={{ fontWeight:900, color:'#0f172a', fontSize:'clamp(32px,4vw,52px)', letterSpacing:'-0.03em', marginBottom:12 }}>Choose Your Perfect Plan</h2>
          ...
          {plans.map((p,i) => (
            <Reveal key={p.label} delay={i*80}>
              <Link to={p.href} style={{ display:'block', height:'100%', textDecoration:'none' }}>
                <div className="glass-card" style={{ padding:36, height:'100%', position:'relative', overflow:'hidden', cursor:'pointer' }}>
                  <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:`linear-gradient(90deg, ${p.accent}, ${p.accent}80)`, borderRadius:'20px 20px 0 0' }} />
                  <div style={{ width:52, height:52, borderRadius:14, background:p.bg, border:`1.5px solid ${p.border}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, marginBottom:20 }}>{p.icon}</div>
                  <h3 style={{ fontSize:20, fontWeight:800, color:'#0f172a', marginBottom:8 }}>{p.label}</h3>
                  <p style={{ fontSize:14, color:'#64748b', lineHeight:1.6, marginBottom:16 }}>{p.desc}</p>
                  <div style={{ fontSize:16, fontWeight:800, color:p.accent, marginBottom:24 }}>{p.price}</div>
                  <ul style={{ listStyle:'none', padding:0, margin:'0 0 28px', display:'flex', flexDirection:'column', gap:10 }}>
                    {p.features.map(f => (
                      <li key={f} style={{ display:'flex', alignItems:'center', gap:10, fontSize:14, color:'#475569' }}>
                        <span style={{ width:18, height:18, borderRadius:'50%', background:p.bg, border:`1.5px solid ${p.border}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, color:p.accent, flexShrink:0, fontWeight:700 }}>✓</span>
                        {f}
                      </li>
                    ))}
                  </ul>
                  <span style={{ fontSize:14, fontWeight:700, color:p.accent }}>View plans →</span>
                </div>
              </Link>
            </Reveal>
          ))}
    ...
    <section style={{ background:'#f8fafc', padding:'96px 24px' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <Reveal><div style={{ textAlign:'center', marginBottom:56 }}>
          <div className="eyebrow" style={{ justifyContent:'center', marginBottom:12 }}>Why JabaliCloud</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:16 }}>
          {/* Large hero card — brand gradient */}
          <Reveal style={{ gridColumn:'span 3' }} delay={0}>
            <div className="glass-card" style={{ padding:44, height:'100%', minHeight:300, background:'linear-gradient(135deg,#2563eb,#00b894)', border:'none', boxShadow:'0 8px 40px rgba(37,99,235,.25)' }}>
              <div style={{ fontSize:44, marginBottom:20 }}>⚡</div>
              <h3 style={{ fontSize:24, fontWeight:900, color:'#fff', letterSpacing:'-0.02em', marginBottom:10 }}>LiteSpeed Powered</h3>
              <p style={{ fontSize:15, color:'rgba(255,255,255,.7)', lineHeight:1.65, marginBottom:28 }}>
                Up to 6× faster than Apache. Sub-second page loads for your visitors, every single time.
              </p>
              <div style={{ background:'rgba(255,255,255,.12)', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:12, color:'rgba(255,255,255,.7)', marginBottom:8 }}>
                  <span>Avg page load speed</span><span style={{fontWeight:700,color:'#fff'}}>0.4s</span>
                </div>
                <div style={{ height:6, borderRadius:99, background:'rgba(255,255,255,.15)', overflow:'hidden' }}>
                  <div style={{ height:'100%', width:'94%', borderRadius:99, background:'rgba(255,255,255,.7)' }} />
                </div>
              </div>
            </div>
          </Reveal>
          ...
          <Reveal style={{ gridColumn:'span 2' }} delay={200}>
            <div className="glass-card" style={{ padding:32, borderColor:'#a7f3d0', background:'linear-gradient(135deg,#ecfdf5,#fff)' }}>
              ...
              <p style={{ fontSize:14, color:'#64748b', lineHeight:1.6 }}>SA primary servers. Edge nodes in Nairobi, Lagos, and Cairo for continental coverage.</p>
            </div>
          </Reveal>
        </div>
      </div>
    ...
    <section style={{ background:'#fff', padding:'96px 24px' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <Reveal><div style={{ textAlign:'center', marginBottom:48 }}>
          <div className="eyebrow" style={{ justifyContent:'center', marginBottom:12 }}>Shared Hosting</div>
          ...
          <p style={{ fontSize:17, color:'#475569', marginBottom:28 }}>Cancel anytime · 30-day money-back guarantee</p>
            ...
            {[['Monthly',false],['Annual',true]].map(([label,val]) => (
              <button key={label} onClick={()=>setAnnual(val)}
                style={{ padding:'8px 20px', borderRadius:99, border:'none', cursor:'pointer', fontSize:13, fontWeight:600, transition:'all .2s',
                  ...
                  color: annual===val ? '#0f172a' : '#64748b',
            ...
            <Reveal key={p.name} delay={i*80}>
              <div style={{ position:'relative', borderRadius:20, height:'100%' }}>
                ...
                <div className="glass-card" style={{ padding:36, height:'100%', display:'flex', flexDirection:'column',
                  ...(p.pop ? { borderColor:'rgba(37,99,235,.25)', background:'linear-gradient(135deg,#eff6ff,#ecfdf5)' } : {}) }}>
                  {p.pop && <div style={{ alignSelf:'flex-start', marginBottom:16, padding:'4px 14px', borderRadius:99, background:'linear-gradient(135deg,#2563eb,#00b894)', color:'#fff', fontSize:11, fontWeight:700 }}>Most Popular</div>}
  ...
  return (
    <section style={{ padding:'96px 24px', background:'linear-gradient(135deg,#1e3a8a,#2563eb 40%,#00b894)' }}>
      <div style={{ maxWidth:860, margin:'0 auto', textAlign:'center' }}>
        <Reveal>
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(255,255,255,.12)', border:'1px solid rgba(255,255,255,.2)', borderRadius:999, padding:'8px 18px', marginBottom:24 }}>
            <span style={{ fontSize:13, fontWeight:500, color:'rgba(255,255,255,.85)' }}>🔍 Domain Registration</span>
          </div>
          ...
          <p style={{ fontSize:17, color:'rgba(255,255,255,.65)', marginBottom:36 }}>Search millions of domains. Competitive prices, instant activation.</p>
        </Reveal>
        <Reveal delay={100}>
          <div style={{ display:'flex', gap:10, maxWidth:600, margin:'0 auto 24px' }}>
            <input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')search();}}
              placeholder="yourbusiness.com"
              style={{ flex:1, padding:'15px 20px', fontSize:15, background:'rgba(255,255,255,.15)', border:'1.5px solid rgba(255,255,255,.25)', borderRadius:12, color:'#fff', outline:'none', fontFamily:'inherit', fontWeight:500 }} />
            <button onClick={search} disabled={busy}
              style={{ padding:'15px 28px', background:'#fff', color:'#1e3a8a', border:'none', borderRadius:12, fontWeight:700, fontSize:14, cursor:'pointer', flexShrink:0, transition:'opacity .15s' }}>
              {busy?'...':'Search'}
            </button>
          </div>
        </Reveal>
        {res.length>0 && (
          <Reveal>
            <div style={{ background:'#fff', borderRadius:16, overflow:'hidden', maxWidth:600, margin:'0 auto 24px', textAlign:'left' }}>
              {res.slice(0,6).map((r,i)=>(
                <div key={r.name} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px', borderBottom:i<5?'1px solid #f1f5f9':'none' }}>
        ...
        <Reveal delay={200}>
          <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:8 }}>
            {TLDS.map(t=>(
              <div key={t} style={{ padding:'8px 16px', background:'rgba(255,255,255,.1)', border:'1px solid rgba(255,255,255,.15)', borderRadius:10 }}>
...
function Testimonials() {
  const items=[
    {name:'Sipho Dlamini',role:'E-commerce Owner · Johannesburg',text:'Switched to JabaliCloud and my load time dropped 60%. Their team actually picks up the phone!'},
    {name:'Amara Osei',role:'Web Developer · Accra',text:'I host 40+ client sites here. Rock-solid uptime, clean cPanel, and best African hosting prices I\'ve found.'},
    {name:'Fatima Al-Hassan',role:'Blogger · Nairobi',text:'One-click WordPress install and free migration — I was live in under an hour with zero tech knowledge.'},
    {name:'David Eze',role:'Startup Founder · Lagos',text:'Best infrastructure decision for my startup. Professional grade at a price that makes sense for our market.'},
  ];
  return (
    <section style={{ background:'#f8fafc', padding:'96px 24px' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <Reveal><div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:16, marginBottom:48 }}>
          <div>
            <div className="eyebrow" style={{ marginBottom:12 }}>Customer Stories</div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            <span style={{ fontSize:14, fontWeight:600, color:'#94a3b8', marginLeft:6 }}>4.9 / 5</span>
          </div>
        </div></Reveal>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        <Reveal>
          <div style={{ background:'#0f172a', borderRadius:28, padding:'80px 64px', textAlign:'center', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ position:'absolute', top:0, left:'50%', transform:'translateX(-50%)', width:400, height:300, pointerEvents:'none', background:'radial-gradient(circle, rgba(37,99,235,.18) 0%, transparent 70%)', filter:'blur(30px)' }} />
            <div style={{ position:'relative' }}>
              <div className="eyebrow" style={{ justifyContent:'center', marginBottom:16, color:'#34d399' }}>Start Today</div>
              <h2 style={{ fontWeight:900, color:'#fff', fontSize:'clamp(32px,5vw,60px)', letterSpacing:'-0.04em', lineHeight:.95, marginBottom:16 }}>
                Launch Your Website<br /><span className="grad">in Minutes.</span>
              </h2>
              <p style={{ fontSize:18, color:'rgba(255,255,255,.45)', maxWidth:480, margin:'0 auto 40px', lineHeight:1.6 }}>
